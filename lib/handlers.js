...
export async function transcribeHandler(req, res) {
  try {
    ...
    if (buffer.length > 20 * 1024 * 1024) {
      return res.status(413).json({ error: "Audio is too large (max ~20MB)." });
    }
...
async function hordeStatus(id) {
  try {
    const r = await fetch(`https://aihorde.net/api/v2/generate/status/${id}`, { signal: AbortSignal.timeout(15000) });
    const j = await r.json().catch(() => ({}));
    if (j.faulted) return { done: false, error: "the free image queue dropped this job — please retry" };
    if (!j.done) return { done: false, wait: j.wait_time, queue: j.queue_position };
    const img = j.generations?.[0]?.img;
    if (!img) return { done: false, error: "the free image queue is overloaded — please retry" };
    try {
      const ir = await fetch(img, { signal: AbortSignal.timeout(20000) });
