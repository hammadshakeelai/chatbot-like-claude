...
export function getClientFor(name, modelOverride) {
  ...
  // timeout guards against a hung upstream; maxRetries:0 keeps failover snappy
  // (we fail over across keys/providers ourselves instead of waiting on retries).
  const client = new OpenAI({ apiKey: apiKey || "missing", baseURL: provider.baseURL, timeout: 30000, maxRetries: 0 });
...
export function clientsFor(name, modelOverride) {
  ...
  return usable.map((apiKey) => ({
    client: new OpenAI({ apiKey: apiKey || "missing", baseURL: provider.baseURL, timeout: 30000, maxRetries: 0 }),
    model,
    providerName,
  }));
}
